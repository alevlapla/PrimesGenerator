 ...
 * числа: 1) значение нечётного числа на индексе i: 2 * i + 3. 2) индекс нечётного
 * полного квадрата: (x^0.5 - 3) / 2. 3) индекс "проброшенного" числа pr с
 * позиции i на позицию i*i: i + (pr * (i + 1)). 4) индекс "проброшенного" числа
 * pr с позиции i на позицию 2*pr: i + pr.
 * 
	...
	public void run() {
		for (;;) {
			...
			if (cand[curr][0] == 0) {
				...
				// Не будем "пробрасывать", если будет выход за пределы массива
				// Здесь мы ограничены переполнением int. Т. к. отследить переполнение
				// int в нашем случае невозможно - будем вычислять в long (предполагаем, 
				// что результат заведомо размещается)			
				long next_index = curr + cand[curr][0] * (curr + 1L);
				if (next_index < size) {
					cand[(int) next_index][0] = temp;
				}
				count++;

				Starter.logger.info("PRIME={}, count={}, curr={}", temp, count, curr);
			} else {
				...
				for (int i = 0; i < cand[curr].length; i++) {
					...
					// int в нашем случае невозможно - будем вычислять в long (предполагаем, 
					// что результат заведомо размещается)
					long next_index = curr + (long) cand[curr][i];
					if (next_index < size) {
			...
			curr++; // Переход к следующему нечётному кандидату на простое 
