...
public class BitSievePageSegmented implements Iterator<Long> {
		...
		if (bi < 1) {
			...
			/* Для страниц после нулевой: № 1, 2, 3... обработка иная (lowi > 0). */
				...
				for (int i = 0; i < bpa.size() - 1; i++) {
					...
					Starter.logger.info("Проход bpa: i={}, простое число p={}, индекс его квадрата s={}, lowi={}", i, p, s,
							lowi);
					/*
					...
					if (s >= lowi) {
						...
						Starter.logger.info("Определение смещения индекса начала высеивания для p={}, s={}, lowi={}.", p,
								s, lowi);

						long r = (lowi - s) % p;
						s = (r != 0) ? p - r : 0;

						Starter.logger.info("                               ...смещение индекса p={}, s={}, lowi={}.", p,
								s, lowi);
	...
	private int countPage(long bitlmt, byte[] buf) {
		...
		for (int i = 0, j = 0; i < bufsz; i++, j = i >> 2) {
			// Необходима маска 0xFF для отбрасывания дополнительного кода отрицательного
			// числа.
		 ...
		 * элемент int[] прилось два элемента byte[], а не четыре).
		 */
		int cnt = (lst << 5) + 32;
		 ...
		 * случай, если последний элемент int[] заполнился не полностью (например, на
		 * последний элемент int[] пришлось два элемента byte[], вместо четырёх).
		 * 
		 * Маска 0xFFFF_FFFE (представляет int, полностью заполненный установленными
		 * битами, кроме младшего) смещается на
		 */

		// TODO разберись с этой маской типа выбраковка последних бит?
		int n = pg[lst] | (0xFFFF_FFFE << (bitlmt & 31));

//		Starter.logger.info("n={}, биты={}", n, Integer.toBinaryString(n));
		cnt -= CLUT[n & 0xFFFF];
		cnt -= CLUT[n >>> 16];
	...
	// TODO перенеси в другой пакет primesgenerator в пакет com.bithell
	/**
	 * Создаёт таблицу поиска, используемую в методе подсчёта количества простых
	 * чисел на данной странице {@link BitSievePageSegmented#countPage(int, int[]) countPage()}
	 * (counting lookup table, CLUT). Таблица поиска отображает количество
	 * установленных бит ("1") для данного индекса таблицы поиска. Экономит ресурсы,
	 * затрачиваемые на постоянные сдвиговые операции при поиске очередного
	 * неустановленного бита с индексом {@code bi}, которому соответствует очередное
	 * простое число (см. цикл {@code while} в конце метода {@link BitSievePageSegmented#next()}).
	 * Вместо данного побитового перебора страницы, страница будет нарезаться на
	 * участки по 16 бит. Каждый участок будет представлять число. Используя данное
	 * число как индекс массива {@code int[] CLUT} можно определить количество
	 * установленных бит без битовых операций.
	 * 
	 * @return массив-таблица поиска, отображающее количество установленных бит
	 *         для числа-индекса.
	 */
